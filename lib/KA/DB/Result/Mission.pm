package KA::DB::Result::Mission;

use Moose;
no warnings qw(uninitialized);
extends 'KA::DB::Result';
use KA::Util qw(format_date commify randint consolidate_items);
use UUID::Tiny ':std';
use Config::JSON;
...
sub complete {
    ...
    confess [1002, 'Already completed that mission in another zone.']
        if KA->cache->get($self->mission_file_name, $body->empire_id);
    KA->cache->set($self->mission_file_name, $body->empire_id, 1, 60 * 60 * 24 * 30);
...
sub add_next_part {
    ...
    return KA::DB::Result::Mission->initialize($self->zone, $name);
}
...
sub feed_url {
    my ($class, $zone) = @_;
    my $config = KA->config;
    KA->config->get('feeds/url').$class->feed_filename($zone);
}

sub feed_filename {
    my ($class, $zone) = @_;
    return 'missioncommand/'.create_uuid_as_string(UUID_MD5, $zone.KA->config->get('feeds/bucket')).'.rss';
}
...
sub find_body_target {
    ...
    given ($movement->{target}{type}) {
        when ('asteroid') { $body = $body->search({ class => { like => 'KA::DB::Result::Map::Body::Asteroid%'} }) };
        when ('habitable') { $body = $body->search({ class => { like => 'KA::DB::Result::Map::Body::Planet::P%'} }) };
        when ('gas_giant') { $body = $body->search({ class => { like => 'KA::DB::Result::Map::Body::Planet::GasGiant%'} }) };
        when ('space_station') { $body = $body->search({ class => { like => 'KA::DB::Result::Map::Body::Station%'} }) };
    }
