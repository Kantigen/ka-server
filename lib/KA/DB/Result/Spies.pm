...
use KA::Util qw(format_date randint random_element commify);
use DateTime;
...
__PACKAGE__->belongs_to('from_body', 'KA::DB::Result::Map::Body', 'from_body_id');
__PACKAGE__->belongs_to('on_body', 'KA::DB::Result::Map::Body', 'on_body_id');
...
sub offensive_assignments {
    ...
    unless ($self->on_body->isa('KA::DB::Result::Map::Body::Planet::Station')) {
        push @assignments, (
            {
    ...
    if (eval{$self->can_conduct_advanced_missions} or $self->on_body->empire_id == -4) {
        ...
        if ( !($self->on_body->isa('KA::DB::Result::Map::Body::Planet::Station')) and $self->on_body->happiness < 0) {
            push @assignments, (
                {
...
sub defensive_assignments {
    ...
    if ($self->on_body->empire_id eq $self->empire_id) {
        if ($self->on_body->get_building_of_class('KA::DB::Result::Building::IntelTraining')) {
            push @assignments, (
            {
                task        =>'Intel Training',
                recovery    => 0,
        ...
        if ($self->on_body->get_building_of_class('KA::DB::Result::Building::MayhemTraining')) {
            push @assignments, (
            {
        ...
        if ($self->on_body->get_building_of_class('KA::DB::Result::Building::PoliticsTraining')) {
            push @assignments, (
            {
        ...
        if ($self->on_body->get_building_of_class('KA::DB::Result::Building::TheftTraining')) {
            push @assignments, (
            {
                task        =>'Theft Training',
                recovery    => 0,
...
sub is_available {
    ...
    if ($task ~~ ['Idle',
        ...
        if ($task eq 'Intel Training') {
            $train_bld = $self->on_body->get_building_of_class('KA::DB::Result::Building::IntelTraining');
            $tr_skill = "intel_xp";
        }
        elsif ($task eq 'Mayhem Training') {
            $train_bld = $self->on_body->get_building_of_class('KA::DB::Result::Building::MayhemTraining');
            $tr_skill = "mayhem_xp";
        }
        elsif ($task eq 'Politics Training') {
            $train_bld = $self->on_body->get_building_of_class('KA::DB::Result::Building::PoliticsTraining');
            $tr_skill = "politics_xp";
        }
        elsif ($task eq 'Theft Training') {
            $train_bld = $self->on_body->get_building_of_class('KA::DB::Result::Building::TheftTraining');
            $tr_skill = "theft_xp";
        }
        if ($train_bld) {
        ...
        if ($task eq 'Debriefing') {
            ...
            if ($self->empire_id ne $self->on_body->empire_id) {
                ...
                if (!$self->empire->alliance_id || $self->empire->alliance_id != $self->on_body->empire->alliance_id ) {
                    ...
                    if ($self->on_body->isa('KA::DB::Result::Map::Body::Planet::Station')) {
                        $building = 'Module::PoliceStation';
                    }
                    my $gauntlet = $self->on_body->get_building_of_class('KA::DB::Result::Building::'.$building);
                    if (defined $gauntlet) {
...
sub assign {
    ...
    if ($assignment ~~ ['Idle',
        ...
        my $train_bld = $self->on_body->get_building_of_class('KA::DB::Result::Building::IntelTraining');
        if ($train_bld) {
            $self->update;
            return {result => 'Accepted', reason => random_element(\@accepted_responses)};
        }
        ...
        my $train_bld = $self->on_body->get_building_of_class('KA::DB::Result::Building::MayhemTraining');
        if ($train_bld) {
            $self->update;
            return {result => 'Accepted', reason => random_element(\@accepted_responses)};
        }
        ...
        my $train_bld = $self->on_body->get_building_of_class('KA::DB::Result::Building::PoliticsTraining');
        if ($train_bld) {
            $self->update;
            return {result => 'Accepted', reason => random_element(\@accepted_responses)};
        }
        ...
        my $train_bld = $self->on_body->get_building_of_class('KA::DB::Result::Building::TheftTraining');
        if ($train_bld) {
            $self->update;
            return {result => 'Accepted', reason => random_element(\@accepted_responses)};
        }
...
# If the media finds out, even more unhappy.
    ...
    if ($self->on_body->empire_id != $old_empire->id) {
        ...
        my $new_int_min = $self->on_body->get_building_of_class('KA::DB::Result::Building::Intelligence');
...
has home_field_advantage => (
    ...
    default => sub {
        ...
        my $hq = $body->get_building_of_class('KA::DB::Result::Building::'.$building);
        if (defined $hq) {
...
# Check to see if new home has room
    ...
    my $new_int_min = $new_home->get_building_of_class('KA::DB::Result::Building::Intelligence');
...
sub gather_resource_intel {
    my $self = shift;
    my $top_choice = $self->on_body->isa('KA::DB::Result::Map::Body::Planet::Station') ? 3 : 5;
...
sub sabotage_defenses {
    my $self = shift;
    my $top_choice = $self->on_body->isa('KA::DB::Result::Map::Body::Planet::Station') ? 3 : 4;
    given (randint(1,$top_choice)) {
...
sub sabotage_resources {
    my $self = shift;
    my $top_choice = $self->on_body->isa('KA::DB::Result::Map::Body::Planet::Station') ? 4 : 9;
    given (randint(1,$top_choice)) {
        when (1) { return $self->knock_defender_unconscious(@_) }
...
sub can_conduct_advanced_missions {
    ...
    return 1 if (KA->config->get('ignore_advanced_mission_limits'));
...
sub steal_planet {
    ...
    if ($planet_happiness > 0 || $failure) { # lose
        ...
        # withdraw trades
        for my $market ( KA->db->resultset('Market'),
                         KA->db->resultset('MercenaryMarket') ) {
    ...
    my $def_capitol = KA->db
                            ->resultset('Map::Body')
                            ->find($defender_capitol_id);

    my $def_int_cap = $def_capitol->get_building_of_class('KA::DB::Result::Building::Intelligence');
    ...
    my @bld_timers = grep {
            ...
            ($_->class eq 'KA::DB::Result::Building::Intelligence')
        }
...
sub destroy_infrastructure {
    ...
    my ($building) = shuffle grep {
            $_->efficiency > 0 and
            $_->class ne 'KA::DB::Result::Building::PlanetaryCommand' and
            $_->class ne 'KA::DB::Result::Building::Module::StationCommand' and
...
sub steal_building {
    ...
    my ($building) = shuffle grep {
            ($_->level > 1) and
            ($_->class ne 'KA::DB::Result::Building::Permanent::EssentiaVein') and
            ($_->class ne 'KA::DB::Result::Building::Permanent::TheDillonForge') and
            ($_->class ne 'KA::DB::Result::Building::Permanent::Fissure') and
            !($_->class =~ /^KA::DB::Result::Building::LCOT/) and
            ($_->class ne 'KA::DB::Result::Building::DeployedBleeder')
        }
...
sub shut_down_defenses {
    ...
    my ($building) = shuffle grep {
            ($_->efficiency > 0) and (
              $_->class eq 'KA::DB::Result::Building::SAW' or
              $_->class eq 'KA::DB::Result::Building::Shipyard' or
              $_->class eq 'KA::DB::Result::Building::Intelligence' or
              $_->class eq 'KA::DB::Result::Building::Security' or
              $_->class eq 'KA::DB::Result::Building::Module::PoliceStation' or
              $_->class eq 'KA::DB::Result::Building::Permanent::GratchsGauntlet' or
              $_->class eq 'KA::DB::Result::Building::Permanent::CitadelOfKnope' )
            }
            @{$self->on_body->building_cache};
...
sub shut_down_building {
    ...
    my ($building) = shuffle grep {
            ($_->efficiency > 0) and (
              $_->class eq 'KA::DB::Result::Building::Archaeology' or
              $_->class eq 'KA::DB::Result::Building::Development' or
              $_->class eq 'KA::DB::Result::Building::Embassy' or
              $_->class eq 'KA::DB::Result::Building::EntertainmentDistrict' or
              $_->class eq 'KA::DB::Result::Building::Intelligence' or
              $_->class eq 'KA::DB::Result::Building::Observatory' or
              $_->class eq 'KA::DB::Result::Building::Park' or
              $_->class eq 'KA::DB::Result::Building::SAW' or
              $_->class eq 'KA::DB::Result::Building::Shipyard' or
              $_->class eq 'KA::DB::Result::Building::SpacePort' or
              $_->class eq 'KA::DB::Result::Building::ThemePark' or
              $_->class eq 'KA::DB::Result::Building::Trade' or
              $_->class eq 'KA::DB::Result::Building::Transporter' or
              $_->class eq 'KA::DB::Result::Building::Waste::Recycling' or
              $_->class eq 'KA::DB::Result::Building::Module::ArtMuseum' or
              $_->class eq 'KA::DB::Result::Building::Module::CulinaryInstitute' or
              $_->class eq 'KA::DB::Result::Building::Module::OperaHouse' or
              $_->class eq 'KA::DB::Result::Building::Module::IBS' or
              $_->class eq 'KA::DB::Result::Building::Module::Warehouse' )
            }
            @{$self->on_body->building_cache};
...
sub network19_propaganda1 {
    ...
    if ($self->on_body
          ->add_news(50,
                     'A resident of %s has won the KA Expanse talent competition.',
                     $self->on_body->name)) {
