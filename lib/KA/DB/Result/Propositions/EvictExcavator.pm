package KA::DB::Result::Propositions::EvictExcavator;
...
before pass => sub {
    my ($self) = @_;
    my $excav = KA->db->resultset('KA::DB::Result::Excavators')->find($self->scratch->{excav_id});   
    ...
    if (! defined $excav) {
        ...
        $excav->planet->get_building_of_class('KA::DB::Result::Building::Archaeology')->remove_excavator($excav);
    }
};
