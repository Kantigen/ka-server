package KA::DB::Result::Proposition::EvictMiningPlatform;
...
before pass => sub {
    my ($self) = @_;
    my $platform = KA->db->resultset('MiningPlatform')->find($self->scratch->{platform_id});   
    ...
    if (! defined $platform) {
        ...
        $platform->planet->get_building_of_class('KA::DB::Result::Building::Ore::Ministry')->remove_platform($platform);
    }
};
