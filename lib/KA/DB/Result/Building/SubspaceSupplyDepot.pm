package KA::DB::Result::Building::SubspaceSupplyDepot;
...
use constant controller_class => 'KA::RPC::Building::SubspaceSupplyDepot';
...
around can_build => sub {
    ...
    confess [1013,"You can't build a Subspace Supply Depot. You have to be given one from KA Expanse Corp."];
};

sub can_upgrade {
    confess [1013, "You can't upgrade a Subspace Supply Depot. You have to be given one from KA Expanse Corp."];
}
...
after finish_work => sub {
    ...
    $self->update({class=>'KA::DB::Result::Building::Permanent::Crater'});
};
