package KA::DB::Result::Building::Module;
...
around spend_efficiency => sub {
    ...
    if ($self->efficiency <= $amount) {
        if ($self->level <= 1 && eval{$self->can_demolish}) {
            ...
            if (!KA->cache->get('downgrade',$self->id)) {
                $self->downgrade;
                KA->cache->set('downgrade',$self->id, 1, 5 * 60);
            }
            else {
...
before can_upgrade => sub {
    ...
    if (defined $plan) {
        ...
        if ($command->level >= $self->level + 1 || $self->isa('KA::DB::Result::Building::Module::StationCommand')) {
            return 1;
        }
        else {
...
sub can_build_on {
    my $self = shift;
    unless ($self->body->isa('KA::DB::Result::Map::Body::Planet::Station')) {
