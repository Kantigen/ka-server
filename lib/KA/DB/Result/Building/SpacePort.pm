package KA::DB::Result::Building::SpacePort;

use Moose;
use utf8;
no warnings qw(uninitialized);
extends 'KA::DB::Result::Building';
use KA::Constants qw(SHIP_TYPES WASTE_F);
...
before delete => sub {
  ...
  unless (KA->db->resultset('KA::DB::Result::Building')
                ->search( { class => $self->class,
                            ...
                            id => {'!=', $self->id } } )->count) {
    my $markets = [
                    { market => 'KA::DB::Result::Market',
                      search => { body_id => $self->body_id, transfer_type => 'trade' }
                    },
                    { market => 'KA::DB::Result::MercenaryMarket',
                      search => { body_id => $self->body_id }
                    },
                  ];

    for my $market_hash ( @{$markets} ) {
      my $market = KA->db->resultset($market_hash->{market});
...
use constant controller_class   => 'KA::RPC::Building::SpacePort';
