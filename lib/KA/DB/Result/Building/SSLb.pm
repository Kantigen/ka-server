package KA::DB::Result::Building::SSLb;
...
# use constant building_prereq => {'KA::DB::Result::Building::SSLa'=>1};
use constant max_instances_per_planet => 1;

use constant controller_class => 'KA::RPC::Building::SSLb';
...
before 'can_demolish' => sub {
    my $self = shift;
    my $sslc = $self->body->get_building_of_class('KA::DB::Result::Building::SSLc');
    if (defined $sslc) {
...
before can_build => sub {
    my $self = shift;
    my $ssla = $self->body->get_building_of_class('KA::DB::Result::Building::SSLa');
    unless (defined $ssla) {
