package KA::DB::Result::Building::Trade;
...
use Data::Dumper;
use KA::Constants qw(SHIP_TRADE_TYPES SHIP_WASTE_TYPES);
...
use constant controller_class => 'KA::RPC::Building::Trade';
...
sub supply_chains {
    ...
    return KA->db->resultset('SupplyChain')->search({ planet_id => $self->body_id });
}
...
sub waste_chains {
    ...
    my $waste_chain = KA->db->resultset('WasteChain')->search({ planet_id => $self->body_id });
    if ($waste_chain->count == 0) {
        KA->db->resultset('WasteChain')->create({
    ...
    return KA->db->resultset('WasteChain')->search({ planet_id => $self->body_id });
}
...
sub add_waste_chain {
    ...
    KA->db->resultset('WasteChain')->new({
        planet_id   => $self->body_id,
        star_id     => $star->id,
...
sub add_to_market {
    ...
    unless(KA::Role::Trader::OVERLOAD_ALLOWED()) {
...
sub push_items {
    ...
    foreach my $item (@{$items}) {
        if ( $item->{type} eq 'fleet' ) {
            my $push_fleet = KA->db->resultset('Fleet')->find($item->{fleet_id});
