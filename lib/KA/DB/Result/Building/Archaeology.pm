package KA::DB::Result::Building::Archaeology;
...
use constant controller_class => 'KA::RPC::Building::Archaeology';
...
sub excavators {
  my $self = shift;
  return KA->db->resultset('KA::DB::Result::Excavators')->search({ planet_id => $self->body_id });
}
...
sub add_excavator {
  my ($self, $body) = @_;
  KA->db->resultset('KA::DB::Result::Excavators')->new({
...
sub make_plan {
    ...
    my %count;
    if ( grep /\D/, @{$glyphs} ) {
        $plan_class = KA::DB::Result::Plan->check_glyph_recipe($glyphs);
        if (not $plan_class) {
