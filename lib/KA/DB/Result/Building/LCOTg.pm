package KA::DB::Result::Building::LCOTg;
...
use constant controller_class => 'KA::RPC::Building::LCOTg';
use constant image => 'lcotc';
use constant name => 'Lost City of Tyleon (G)';

before 'can_demolish' => sub {
    my $self = shift;
    my $h = $self->body->get_building_of_class('KA::DB::Result::Building::LCOTh');
    if (defined $h) {
