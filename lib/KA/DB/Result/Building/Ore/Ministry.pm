package KA::DB::Result::Building::Ore::Ministry;

use Moose;
use utf8;
no warnings qw(uninitialized);
extends 'KA::DB::Result::Building::Ore';
use KA::Constants qw(ORE_TYPES GROWTH_F INFLATION_N CONSUME_S WASTE_F);
use POSIX qw(ceil);
...
sub platforms {
    my $self = shift;
    return KA->db->resultset('MiningPlatform')->search({ planet_id => $self->body_id });
}
...
sub can_add_platform {
    ...
    # asteroid count
    $count = KA->db->resultset('MiningPlatform')->search({ asteroid_id => $asteroid->id })->count;
    unless ($on_arrival) {
...
sub add_platform {
    my ($self, $asteroid) = @_;
    KA->db->resultset('MiningPlatform')->new({
...
use constant controller_class => 'KA::RPC::Building::MiningMinistry';
