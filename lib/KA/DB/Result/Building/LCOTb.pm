package KA::DB::Result::Building::LCOTb;
...
use constant controller_class => 'KA::RPC::Building::LCOTb';
use constant image => 'lcotb';
use constant name => 'Lost City of Tyleon (B)';

before 'can_demolish' => sub {
    my $self = shift;
    my $c = $self->body->get_building_of_class('KA::DB::Result::Building::LCOTc');
    if (defined $c) {
...
before can_build => sub {
    my $self = shift;
    my $a = $self->body->get_building_of_class('KA::DB::Result::Building::LCOTa');
    unless (defined $a) {
