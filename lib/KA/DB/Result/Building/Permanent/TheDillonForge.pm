package KA::DB::Result::Building::Permanent::TheDillonForge;
...
use constant controller_class => 'KA::RPC::Building::TheDillonForge';
...
sub split_plan {
    ...
    my $class   = 'KA::DB::Result::Building::'.$plan_class;
    my $body    = $self->body;
    my ($plan) = grep {
            $_->level               == $level
    ...
    my $glyphs = KA::DB::Result::Plan->get_glyph_recipe($class);
    if (not $glyphs) {
...
sub working_forges {
    ...
    my $working_forges = KA->db->resultset('Empire')
        ->search(
                 {
                     '_buildings.class' => "KA::DB::Result::Building::Permanent::TheDillonForge",
...
before finish_work => sub {
    ...
    if ($work->{task} eq 'split_plan') {
        ...
        my $glyphs = KA::DB::Result::Plan->get_glyph_recipe($plan_class);
        my $glyphs_built;
        my $total_glyphs = 0;
        for my $glyph (@$glyphs) {
