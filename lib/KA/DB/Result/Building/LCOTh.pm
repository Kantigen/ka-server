package KA::DB::Result::Building::LCOTh;
...
use constant controller_class => 'KA::RPC::Building::LCOTh';
use constant image => 'lcotd';
use constant name => 'Lost City of Tyleon (H)';

before 'can_demolish' => sub {
    my $self = shift;
    my $i = $self->body->get_building_of_class('KA::DB::Result::Building::LCOTi');
    if (defined $i) {
