package KA::DB::Result::Building::LCOTf;
...
use constant controller_class => 'KA::RPC::Building::LCOTf';
use constant image => 'lcotf';
use constant name => 'Lost City of Tyleon (F)';

before 'can_demolish' => sub {
    my $self = shift;
    my $g = $self->body->get_building_of_class('KA::DB::Result::Building::LCOTg');
    if (defined $g) {
...
before can_build => sub {
    my $self = shift;
    my $e = $self->body->get_building_of_class('KA::DB::Result::Building::LCOTe');
    unless (defined $e) {
