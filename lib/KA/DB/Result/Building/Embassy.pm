package KA::DB::Result::Building::Embassy;
...
use constant controller_class => 'KA::RPC::Building::Embassy';
...
sub update_alliance {
    ...
    foreach my $field (qw(forum_uri description announcements)) {
        ...
        KA::Verify->new(content=>\$content, throws=>[1005,'The '.$field.' must not contain restricted characters or profanity.', $field])
            ->no_tags
            ->no_profanity;
...
has exchanges_remaining_today => (
    ...
    default => sub {
        my $self = shift;
        return $self->effective_level - KA->cache->get('stash_exchanges_'.format_date(undef,'%d'), $self->body_id);
    },
);
