package KA::DB::Result::Map::Body::Planet::Station;
...
use KA::Constants qw(MINIMUM_EXERTABLE_INFLUENCE);
use Data::Dumper;
...
before sanitize => sub {
    ...
    KA->db->resultset('StationInfluence')->search({ station_id => $self->id })->delete;
    KA->db->resultset('Map::Star')->search({ station_id => $self->id })->update({ station_id => undef });
};
...
after sanitize => sub {
    ...
    $self->update({
        size        => randint(1,10),
        class       => 'KA::DB::Result::Map::Body::Asteroid::A21',
        alliance_id => undef,
    });
};
...
sub _build_total_influence {
    ...
    foreach my $building (@{$self->building_cache}) {
        if (
            $building->class eq 'KA::DB::Result::Building::Module::IBS' or
            $building->class eq 'KA::DB::Result::Building::Module::OperaHouse' or
            $building->class eq 'KA::DB::Result::Building::Module::CulinaryInstitute' or
            $building->class eq 'KA::DB::Result::Building::Module::ArtMuseum'
            ) {
...
sub _build_range_of_influence {
    ...
    my $range = 0;
    my ($ibs) = grep {$_->class eq 'KA::DB::Result::Building::Module::IBS'} @{$self->building_cache};
    if (defined $ibs) {
        $range = $ibs->level * 300;
    }
    return $range;
...
sub update_influence {
    ...
    if ($centerpoint->in_neutral_area() or $centerpoint->in_starter_zone())
    {
        ...
        my $si = KA->db->resultset('StationInfluence')->search({station_id => $self->id})->delete;
        return;
    }
...
EOSQL
    ...
    if (@$all_ids)
    {
        my $si = KA->db->resultset('StationInfluence');
    ...
    my $home_star = KA->db->resultset('StationInfluence')->find({station_id => $self->id, star_id => $self->star_id});
    if (!$home_star || $calc_influence->(0) != $home_star->influence )
    {
        my $rs = KA->db->resultset('StationInfluence');
        my $influence_rs = $rs->
            search(
    ...
    my $stars_rs = KA->db->resultset('Map::Star')->search({
        -and => [
    ...
    while (my $star = $stars_rs->next)
    {
        ...
        {
            my $inf = KA->db->resultset('StationInfluence')->
                create(
                       {
                           station_id        => $self->id,
                           star_id           => $star->id,
