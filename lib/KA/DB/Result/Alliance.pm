...
__PACKAGE__->belongs_to('leader', 'KA::DB::Result::Empire', 'leader_id', { on_delete => 'set null' });
__PACKAGE__->has_many('members', 'KA::DB::Result::Empire', 'alliance_id');
__PACKAGE__->has_many('invites', 'KA::DB::Result::AllianceInvite', 'alliance_id');
__PACKAGE__->has_many('stations', 'KA::DB::Result::Map::Body', 'alliance_id');
...
sub send_invite {
    ...
    my $invite = KA->db->resultset('AllianceInvite')->new({
        alliance_id => $self->id,
        empire_id   => $empire->id,
    })->insert;
