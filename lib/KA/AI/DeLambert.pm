...
sub sell_glyph_trade {
    ...
    my $scratchpad = $self->scratch->pad;
    my $trade_min = $colony->get_building_of_class('KA::DB::Result::Building::Trade');
...
sub sell_plan_trade {
    ...
    if (randint(1,100) <= $scratchpad->{sell_plan_probability}) {
        ...
        for (1..$quantity) {
            ...
            push @plans, {
                class               => "KA::DB::Result::Building::Permanent::$type",
                level               => $level,
                quantity            => 1,
                extra_build_level   => 0,
            };
...
We believed that The KA Expanse was a peaceful place where we could trade to mutual benefit
