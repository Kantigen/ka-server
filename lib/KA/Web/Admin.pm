...
use Module::Find;
use UUID::Tiny ':std';
use KA::Util qw(format_date commify kmbtq);
...
#        next unless ('Infrastructure' ~~ [$building->build_tags]);
        next if ( $building->class eq 'KA::DB::Result::Building::PlanetaryCommand' );
        $building->class('KA::DB::Result::Building::Permanent::Crater');
        $building->level(1);
...
sub www_view_buildings {
    ...
    my %buildings = map { $_->name => $_ } findallmod KA::DB::Result::Building;
...
sub www_view_fleets {
    my ($self, $request, $body_id) = @_;
    $body_id ||= $request->param('body_id');
    my $fleets = KA->db->resultset('Fleet')->search({ body_id => $body_id });
...
sub www_delete_fleet {
    my ($self, $request) = @_;
    my $fleet = KA->db->resultset('Fleet')->find($request->param('fleet_id'));
    $fleet->delete;
    return $self->www_view_fleets($request);
}
...
sub www_become_empire {
    ...
    my $uri = KA->config->get('server_url');
    $uri .= '#session_id=%s';
...
sub www_set_admin_notes {
    ...
    my $empire = KA->db->empire($id);
    my $notes = decode_utf8($request->param('notes'));

    my $note = KA->db->resultset('Log::EmpireAdminNotes')->new({
                       empire_id   => $empire->id,
...
sub www_view_admin_note_log {
    ...
    my $history = KA->db->resultset('Log::EmpireAdminNotes')->search({empire_id => $empire->id},{order_by => { -desc => 'date_stamp' }});
...
sub www_set_alliance_logo {
    ...
    my $full_url = KA->config->get('assets_url').'alliances/'.$image.'.png';
...
sub www_default {
    my ($self, $request) = @_;
    my $announcement = KA->cache->get('announcement','message');
    $announcement =~ s/\>/&gt;/xmsg;
    $announcement =~ s/\</&lt;/xmsg;
    return $self->wrap('<h1>KA Expanse Admin Console</h1>
            Server Version: '.KA->version.'
        <ul>
