...
sub www_postback {
    ...
    my $empire = KA->db->resultset('KA::DB::Result::Empire')->search({facebook_uid => $user->{id} })->first;
...
sub www_my_empire {
    ...
    unless (defined $empire) {
        ...
        return $self->wrapper($out, { title => 'Play for Free in The KA Expanse' });   
    }
    ...
    return $self->wrapper($out, { title => 'My Empire in The KA Expanse', logo => 1 });
}
