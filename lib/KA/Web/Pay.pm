...
sub www_default {
    ...
    KA->cache->set( 'paypal_order', $empire->id, { session => $session_id, }, 60 * 30 );
...
sub www_buy_currency_cc {
    ...
    my $config = KA->config->get('itransact');
...
sub paypal_ec_cancel {
    my ($self, $user_id) = @_;
    my $config = KA->config->get();
    KA->cache->delete( 'paypal_order', $user_id );
...
sub www_buy_currency_paypal {
    ...
    my $config = KA->config->get('paypal');
warn Dumper( $config );
...
#        InvoiceID   => $InvoiceID,
    ...
    my $order = KA->cache->get_and_deserialize( 'paypal_order', $user_id );
    KA->cache->set( 'paypal_order', $user_id, { session => $order->{session}, token => $token, total => $total, currency => $currency }, 60 * 30 );
...
sub _run_promotions
{
    ...
    my $rs = KA->db->resultset("Promotion")->current_promotions_rs;
