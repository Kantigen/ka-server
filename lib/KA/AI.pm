...
sub add_colonies {
    ...
    my @all_zones = KA->db->resultset('Map::Star')->search(
                        undef,
                        { distinct => 1 }
                    )->get_column('zone')->all;
    ...
    my $na_param = KA->config->get('neutral_area');
    my @neutral_zones = ();
    if ($na_param->{zone}) {
...
sub demolish_bleeders {
  ...
  my @bleeders = $colony->get_buildings_of_class('KA::DB::Result::Building::DeployedBleeder');
...
sub pod_check {
  ...
  return if (KA->cache->get('supply_pod_sent',$colony->id));
...
# Check to see if spot found, if not, clear off a crater if found.
    unless ($@) {
      ...
      KA->cache->set('supply_pod_sent',$colony->id,1,60*60*24);
    }
    else {
      my @craters = $colony->get_buildings_of_class('KA::DB::Result::Building::Permanent::Crater');
      if (@craters) {
...
sub train_spies {
    ...
    my $intelligence = $colony->get_building_of_class('KA::DB::Result::Building::Intelligence');
...
# Create attack_group
    ...
    my $payload;
    $ag_chk = 0 if ($target_body->isa('KA::DB::Result::Map::Star'));
