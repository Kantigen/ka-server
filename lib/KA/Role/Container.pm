package KA::Role::Container;

use Moose::Role;
use KA::Constants qw(ORE_TYPES FOOD_TYPES);
use KA::Util qw(format_date commify consolidate_items);
...
sub unload {
    ...
    if (exists $payload->{prisoners}) {
        foreach my $id (@{$payload->{prisoners}}) {
            my $prisoner = KA->db->resultset('Spy')->find($id);
    ...
    if (exists $payload->{mercenary}) {
        ...
        my $spy = KA->db->resultset('Spy')->find($id);
    ...
    if (exists $payload->{ships}) {
        foreach my $id (@{$payload->{ships}}) {
            my $fleet = KA->db->resultset('Fleet')->find($id);
...
sub format_description_of_payload {
    ...
    # ships
    my $fleets = KA->db->resultset('Fleet');
