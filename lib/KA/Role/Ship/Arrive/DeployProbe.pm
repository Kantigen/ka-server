package KA::Role::Ship::Arrive::DeployProbe;
...
after can_send_to_target => sub {
    my ($self, $target) = @_;
    my $body = $self->body;
    my $count = KA->db->resultset('KA::DB::Result::Probes')->search({ body_id => $body->id })->count;
    $count += KA->db->resultset('KA::DB::Result::Ships')->search({ body_id => $body->id, type=>'probe', task=>'Travelling' })->count;
    my $max_probes = 0;
