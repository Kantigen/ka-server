package KA::Role::Ship::Arrive::DestroyMinersExcavators;
...
after handle_arrival_procedures => sub {
  ...
  my $is_uninhabit  = ($body_attacked->isa('KA::DB::Result::Map::Body::Planet') and !(defined($body_attacked->empire_id)) );
...
# find mining platforms to destroy
  if ($is_asteroid) {
    my $platforms = KA->db->resultset('KA::DB::Result::MiningPlatforms')->search({asteroid_id => $self->foreign_body_id });
...
# Destroy Excavs
  my $excavs = KA->db->resultset('KA::DB::Result::Excavators')->search({body_id => $self->foreign_body_id });
