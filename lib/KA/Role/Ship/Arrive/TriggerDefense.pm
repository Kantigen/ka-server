package KA::Role::Ship::Arrive::TriggerDefense;
...
after handle_arrival_procedures => sub {
    ...
    my $is_planet = $body_attacked->isa('KA::DB::Result::Map::Body::Planet');
    my $is_asteroid = $body_attacked->isa('KA::DB::Result::Map::Body::Asteroid');
...
sub log_attack {
  ...
  $logs->new({
     ...
     attacking_unit_name     =>
       $attacker->isa('KA::DB::Result::Building') ?
     ...
     defending_unit_name     => $defender->isa('KA::DB::Result::Building') ?
       sprintf("%s (%d,%d)", $defender->name, $defender->x, $defender->y) :
       $defender->name,
     defending_type          => 
       $defender->isa('KA::DB::Result::Building') ?
...
sub defender_combat {
    ...
    # get defensive ships
    my $defense_ships = KA->db->resultset('KA::DB::Result::Ships')->search(
...
sub allied_combat {
    ...
    my $allied_ships = KA->db->resultset('KA::DB::Result::Ships')->search(
		{
...
sub system_saw_combat {
  ...
  my $ship_body = $self->body;
  my $is_planet = $attacked_body->isa('KA::DB::Result::Map::Body::Planet');
  my $is_asteroid = $attacked_body->isa('KA::DB::Result::Map::Body::Asteroid');
  ...
  my $defending_bodies = KA->db->resultset('KA::DB::Result::Map::Body')->search(
...
# 2) Go thru and get all SAWs hostile to incoming.
  ...
  ( $attacked_saws, $attacked_combat ) = $self->saw_stats($attacked_body)
     if ($attacked_body->isa('KA::DB::Result::Map::Body::Planet'));
# Attacked planet defense a bit more fierce.
  ...
  while (my $defending_body = $defending_bodies->next) {
    ...
    next
      unless $defending_body->isa('KA::DB::Result::Map::Body::Planet');
...
sub saw_stats {
    ...
    my @planet_saws = $body->get_buildings_of_class('KA::DB::Result::Building::SAW');
