package KA::Role::Building::UpgradeWithHalls;
...
around can_upgrade => sub {
    ...
    my ($plan) = grep {$_->class eq 'KA::DB::Result::Building::Permanent::HallsOfVrbansk'} @{$body->plan_cache};
...
before start_upgrade => sub {
    ...
    my ($plans) = grep {$_->class eq 'KA::DB::Result::Building::Permanent::HallsOfVrbansk'} @{$self->body->plan_cache};
