package KA::Role::Fleet::Arrive::Colonize;
...
after handle_arrival_procedures => sub {
    ...
    if ($planet->isa('KA::DB::Result::Map::Body::Planet::GasGiant') && $empire->university_level < 19) {
    ...
    elsif ($planet->isa('KA::DB::Result::Map::Body::Asteroid')) {
        $empire->send_predefined_message(
            tags        => ['Colonization','Alert'],
...
after can_send_to_target => sub {
    my ($self, $target) = @_;
    my $empire = $self->body->empire;
    confess [1009, 'Can only be sent to habitable planets.'] if ($target->isa('KA::DB::Result::Map::Body::Planet::GasGiant') && $empire->university_level < 19);
