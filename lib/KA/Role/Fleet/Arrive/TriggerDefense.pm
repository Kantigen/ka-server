package KA::Role::Fleet::Arrive::TriggerDefense;
...
sub defender_combat {
    ...
    my $defense_fleets = KA->db->resultset('Fleet')->search({
...
sub allied_combat {
    ...
    my $allied_fleets = KA->db->resultset('Fleet')->search({
...
sub system_saw_combat {
    ...
    my $defending_bodies = KA->db->resultset('Map::Body')->search({
        id      => { '!=' => $attacked_body->id}, 
    ...
    while (my $defending_body = $defending_bodies->next) {
        ...
        next unless $defending_body->isa('KA::DB::Result::Map::Body::Planet');
