...
sub explore_the_ui {
    ...
    if ($finish) {
        if ($home->name ne $empire->tutorial_scratch) {
            ...
            $home->add_plan('KA::DB::Result::Building::Food::Malcud', 1);
            $self->start('get_food');
            return undef;
        }
    }
...
sub pawn {
    ...
    if ($finish) {
        my $building = $home->get_building_of_class('KA::DB::Result::Building::Intelligence');
        if (defined $building && $building->level >= 1) {
            $home->add_plan('KA::DB::Result::Building::Intelligence', 2);
            $building->train_spy;
...
sub observatory {
    ...
    my $home = $self->empire->home_planet;
    if ($finish) {
        my $building = $home->get_building_of_class('KA::DB::Result::Building::Observatory');
        if (defined $building && $building->level >= 1) {
...
sub explore {
    ...
    if ($finish) {
        if ($empire->count_probed_stars > 1) {
            $empire->home_planet->add_plan('KA::DB::Result::Building::Transporter', 1);
            $self->start('the_end');
            return undef;
        }
    }
...
sub university {
    ...
    if ($finish) {
        ...
        if (defined $building && $building->level >= 1) {
            ...
            $home->add_water(1000);
            $home->update;
            $home->add_plan('KA::DB::Result::Building::MissionCommand', 1);
            foreach my $filename (qw(soylent-trade-opportunity.mission need-a-probe.mission dung-beeldeban.mission)) {
                KA::DB::Result::Mission->initialize($home->zone, $filename);
            }
...
sub storage {
    ...
    my $home = $empire->home_planet;
    if ($finish) {
        my $building = $home->get_building_of_class('KA::DB::Result::Building::Ore::Storage');
        if (defined $building && $building->level >= 1) {
            my $building = $home->get_building_of_class('KA::DB::Result::Building::Water::Storage');
            if (defined $building && $building->level >= 1) {
                my $building = $home->get_building_of_class('KA::DB::Result::Building::Energy::Reserve');
                if (defined $building && $building->level >= 1) {
