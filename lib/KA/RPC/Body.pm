...
use Moose;
use KA::Verify;
use KA::Constants qw(BUILDABLE_CLASSES);
use DateTime;
...
sub get_body_status {
    ...
    my $body    = KA->db->resultset('Map::Body')->find($args{body_id});
...
sub rearrange_buildings {
  ...
  for my $id (keys %new_ids) {
    ...
    $new_lay{$spot} = $id;
    if ($spot eq "0:0") {
      if ($new_ids{$id}->{class} ne "KA::DB::Result::Building::PlanetaryCommand" and
          $new_ids{$id}->{class} ne "KA::DB::Result::Building::Module::StationCommand") {
...
sub check_positions {
  ...
  for my $id (keys %$new_ids) {
    ...
    given ($new_ids->{$id}->{class}) {
      when("KA::DB::Result::Building::PlanetaryCommand") {
      ...
      when("KA::DB::Result::Building::Module::StationCommand") {
        unless ($new_ids->{$id}->{x} == 0 &&
            $new_ids->{$id}->{y} == 0) {
          push @position_err,
               sprintf("%s can not be placed anywhere but 0,0", $new_ids->{$id}->{name});
      ...
      when("KA::DB::Result::Building::SSLa") {
      ...
      when("KA::DB::Result::Building::SSLb") {
      ...
      when("KA::DB::Result::Building::SSLc") {
      ...
      when("KA::DB::Result::Building::SSLd") {
        $spot_chk = sprintf("%d:%d", $new_ids->{$id}->{x} + 1, $new_ids->{$id}->{y});
        if ($new_ids->{$new_lay->{$spot_chk}}->{class} ne
      ...
      when("KA::DB::Result::Building::LCOTa") {
        if ($new_ids->{$id}->{x} ~~ [ -5, 5 ] ||
            $new_ids->{$id}->{y} ~~ [ -5, 5 ] ||
           ($new_ids->{$id}->{x} ~~ [ -1, 0, 1 ] &&
            $new_ids->{$id}->{y} ~~ [ -1, 0, 1 ])) {
      ...
      when("KA::DB::Result::Building::LCOTb") {
        $spot_chk = sprintf("%d:%d", $new_ids->{$id}->{x} + 1, $new_ids->{$id}->{y});
        if ($new_ids->{$new_lay->{$spot_chk}}->{class} ne
      ...
      when("KA::DB::Result::Building::LCOTc") {
        $spot_chk = sprintf("%d:%d", $new_ids->{$id}->{x}, $new_ids->{$id}->{y}-1);
        if ($new_ids->{$new_lay->{$spot_chk}}->{class} ne
      ...
      when("KA::DB::Result::Building::LCOTd") {
        $spot_chk = sprintf("%d:%d", $new_ids->{$id}->{x} - 1, $new_ids->{$id}->{y});
        if ($new_ids->{$new_lay->{$spot_chk}}->{class} ne
      ...
      when("KA::DB::Result::Building::LCOTe") {
        $spot_chk = sprintf("%d:%d", $new_ids->{$id}->{x} - 1, $new_ids->{$id}->{y});
        if ($new_ids->{$new_lay->{$spot_chk}}->{class} ne
      ...
      when("KA::DB::Result::Building::LCOTf") {
        $spot_chk = sprintf("%d:%d", $new_ids->{$id}->{x}, $new_ids->{$id}->{y}+1);
        if ($new_ids->{$new_lay->{$spot_chk}}->{class} ne
      ...
      when("KA::DB::Result::Building::LCOTg") {
        $spot_chk = sprintf("%d:%d", $new_ids->{$id}->{x}, $new_ids->{$id}->{y}+1);
        if ($new_ids->{$new_lay->{$spot_chk}}->{class} ne
      ...
      when("KA::DB::Result::Building::LCOTh") {
        $spot_chk = sprintf("%d:%d", $new_ids->{$id}->{x} + 1, $new_ids->{$id}->{y});
        if ($new_ids->{$new_lay->{$spot_chk}}->{class} ne
      ...
      when("KA::DB::Result::Building::LCOTi") {
        $spot_chk = sprintf("%d:%d", $new_ids->{$id}->{x} + 1, $new_ids->{$id}->{y});
        if ($new_ids->{$new_lay->{$spot_chk}}->{class} ne
    ...
    foreach my $class (uniq @buildable) {
        ...
        $out{$building->name} = {
            ...
            build       => {
                ...
                reason      => $reason,
                tags        => \@tags,
                no_plot_use => $building->isa('KA::DB::Result::Building::Permanent'),
            },
...
sub set_colony_notes
{
    ...
    #KA::Verify->new(content=>\$notes, throws=>[1000,'Content may not have any of the following characters: @&<>;{}()',$notes])
    #    ->no_restricted_chars;
