package KA::RPC::Empire;

use Moose;
use utf8;
no warnings qw(uninitialized);
extends 'KA::RPC';
use KA::Util qw(format_date randint real_ip_address);
use DateTime;
...
sub find {
    ...
    my $empires = KA->db->resultset('Empire')->search({name => {'like' => $name.'%'}}, {rows=>100});
    my @list_of_empires;
    my $limit = 100;
...
sub is_name_valid {
    ...
    KA::Verify->new(content=>\$name, throws=>[1000,'Empire name is invalid.', 'name'])
        ->length_lt(31)
        ->length_gt(2)
        ->not_empty
        ->no_padding
...
sub is_name_unique {
    ...
    if (KA->db->resultset('Empire')->search({name=>$name})->count) {
...
sub login {
    ...
    my $empire;
    if ($name =~ /^#(-?\d+)$/) {
        $empire = KA->db->resultset('Empire')->find({id=>$1});
    }
    else {
        $empire = KA->db->resultset('Empire')->find({name=>$name});
    }
    ...
    if ($empire->is_password_valid($password)) {
        ...
        unless (KA->cache->get('invalid_login_attempt_' . $ip, $empire->id)) {
            KA->cache->set('invalid_login_attempt_' . $ip, $empire->id, 1, 12 * 60 * 60);
...
sub benchmark {
    ...
    my $empire = KA->db->resultset('Empire')->search({name=>$name})->next;
...
sub fetch_captcha {
    ...
    KA->cache->set('create_empire_captcha', $ip, { guid => $captcha->guid, solution => $captcha->solution }, 60 * 15 );
...
sub send_password_reset_message {
    ...
    $empire->send_email(
        'Reset Your Password',
        sprintf($message, $empire->name, $empire->password_recovery_key, KA->config->get('server_url'), $empire->password_recovery_key),
    );
    return { sent => 1 };
}
...
sub reset_password {
    ...
    my $empire = KA->db->resultset('Empire')->search({password_recovery_key => $key})->first;
    unless (defined $empire) {
...
sub create {
    ...
    if (exists $args{password} || !$has_facebook) {
        KA::Verify->new(content=>\$args{password}, throws=>[1001,'Invalid password. It must be at least 6 characters and both passwords must match.', 'password'])
            ->length_gt(5)
            ->eq($args{password1});
    ...
    if ($@) { # maybe they're trying to finish an incomplete empire
        my $empire = KA->db->resultset('Empire')->search({name=>$args{name}})->next;
        if (defined $empire) {
            if ($empire->stage eq 'new') {
    ...
    if (exists $args{email} && $args{email} ne '') {
        KA::Verify->new(content=>\$args{email}, throws=>[1005,'The email address specified does not look valid.', 'email'])
            ->is_email;
        if (KA->db->resultset('Empire')->search({email=>$args{email}})->count > 0) {
...
sub validate_captcha {
    ...
    if (defined $guid && defined $solution) {                                               # offered a solution
        my $captcha = KA->cache->get_and_deserialize('create_empire_captcha', $ip);
...
sub edit_profile {
    ...
    if (exists $args->{status_message}) {
        KA::Verify->new(content=>\$args->{status_message}, throws=>[1005,'Status cannot be empty, must be no longer than 100 characters, and cannot contain special characters or profanity.', 'status_message'])
            ->length_lt(101)
            ->not_empty
            ->no_restricted_chars
            ->no_profanity;
    ...
    if (exists $args->{sitter_password}) {
        KA::Verify->new(content=>\$args->{sitter_password}, throws=>[1005,'Sitter password must be between 6 and 30 characters.', 'sitter_password'])
            ->length_lt(31)
            ->length_gt(5);
        $empire->sitter_password($args->{sitter_password});
    }
    if (exists $args->{city}) {
        KA::Verify->new(content=>\$args->{city}, throws=>[1005,'City must be no longer than 100 characters, and cannot contain special characters or profanity.', 'city'])
            ->length_lt(101)
            ->no_restricted_chars
            ->no_profanity;
    ...
    if (exists $args->{country}) {
        KA::Verify->new(content=>\$args->{country}, throws=>[1005,'Country must be no longer than 100 characters, and cannot contain special characters or profanity.', 'country'])
            ->length_lt(101)
            ->no_restricted_chars
            ->no_profanity;
    ...
    if (exists $args->{player_name}) {
        KA::Verify->new(content=>\$args->{player_name}, throws=>[1005,'Player name must be no longer than 100 characters, and cannot contain special characters or profanity.', 'player_name'])
            ->length_lt(101)
            ->no_restricted_chars
            ->no_profanity;
    ...
    if (exists $args->{skype}) {
        KA::Verify->new(content=>\$args->{skype}, throws=>[1005,'Skype must be no longer than 100 characters, and cannot contain special characters or profanity.', 'skype'])
            ->length_lt(101)
            ->no_restricted_chars
            ->no_profanity;
    ...
    if (exists $args->{email} && $args->{email} ne '') {
        ...
        if (KA->db->resultset('Empire')->search({email=>$args->{email}, id=>{ '!=' => $empire->id}})->count > 0) {
            confess [1005, 'That email address is already in use by another empire.', 'email'];
        }
...
sub set_status_message {
    ...
    KA::Verify->new(content=>\$message, throws=>[1005,'Status message invalid.', 'status_message'])
        ->length_lt(101)
        ->not_empty
        ->no_restricted_chars
        ->no_profanity;
...
sub get_public_profile {
    ...
    my $viewed_empire = KA->db->resultset('Empire')->find($empire_id);
...
sub vet_species {
    ...
    KA::Verify->new(content=>\$args->{name}, throws=>[1000,'Species name not available.', 'name'])
        ->length_lt(31)
        ->length_gt(2)
        ->not_empty
        ->no_restricted_chars
...
sub get_authorized_sitters {
    ...
    my $parser = KA->db->storage->datetime_parser;
...
sub authorize_sitters {
    ...
    if ($args{alliance}) {
        my $alliance = KA->db->resultset('Alliance')->find({name => $args{alliance}});
        if ($alliance) {
            push @sitters, $alliance->members->all;
        }
    }
    if ($args{alliance_id}) {
        my $alliance = KA->db->resultset('Alliance')->find({id => $args{alliance_id}});
        if ($alliance) {
            push @sitters, $alliance->members->all;
        }
    }
    ...
    for my $sitter (@sitters) {
        my $sit = eval { ref $sitter && $sitter->isa('KA::DB::Result::Empire') } ?
            $sitter :
            KA->db->empire($sitter);
        if ($sit) {
            my $sitter_id = $sit->id;
