...
sub messages_rs {
    ...
    my $message = KA->db->resultset('Message')->
        search(
               {
...
sub send_message {
    ...
    KA::Verify->new(content=>\$subject, throws=>[1005,'Message subject cannot be empty.',$subject])->not_empty;
    KA::Verify->new(content=>\$subject, throws=>[1005,'Message subject cannot contain any of these characters: (){}<>&;@',$subject])->no_restricted_chars;
    KA::Verify->new(content=>\$subject, throws=>[1005,'Message subject must be less than 100 characters.',$subject])->length_lt(100);
    KA::Verify->new(content=>\$body, throws=>[1005,'Message body cannot be empty.',$body])->not_empty;
    KA::Verify->new(content=>\$body, throws=>[1005,'Message body cannot contain HTML tags or entities.',$body])->no_tags;
    my $session  = $self->get_session({session_id => $session_id });
    my $empire   = $session->current_empire;
    ...
    foreach my $to (@to) {
        if ($to->id == 1) {
            KA::Tutorial->new(empire=>$empire)->finish(1);
        }
        else {
            $to->send_message(
                from        => $empire,
