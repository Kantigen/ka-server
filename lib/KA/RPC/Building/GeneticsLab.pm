package KA::RPC::Building::GeneticsLab;
...
sub model_class {
    return 'KA::DB::Result::Building::GeneticsLab';
}
...
sub rename_species {
    ...
    KA::Verify->new(content=>\$me->{name}, throws=>[1000,'Species name not available.', 'name'])
        ->length_lt(31)
        ->length_gt(2)
        ->not_empty
        ->no_restricted_chars
    ...
    # and the description
    KA::Verify->new(content=>\$me->{description}, throws=>[1005,'Description invalid.', 'description'])
        ->length_lt(1025)
        ->no_restricted_chars
