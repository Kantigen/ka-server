package KA::RPC::Building::Embassy;
...
sub model_class {
    return 'KA::DB::Result::Building::Embassy';
}
...
sub assign_alliance_leader {
    ...
    my $new_leader = KA->db->resultset('Empire')->find($empire_id);
...
sub expel_member {
    ...
    my $empire   = $session->current_empire;
    my $building = $session->current_building;
    my $member = KA->db->resultset('Empire')->find($member_id);
