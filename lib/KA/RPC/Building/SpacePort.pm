package KA::RPC::Building::SpacePort;
...
sub model_class {
    return 'KA::DB::Result::Building::SpacePort';
}
...
sub _view_available_fleets {
    ...
    my $fleet_rs = KA->db->resultset('Fleet')->search($filter, $attrs);
...
sub prepare_send_spies {
    ...
    if (not $to_body->isa('KA::DB::Result::Map::Body::Planet')) {
...
sub prepare_fetch_spies {
    ...
    if (not $on_body->isa('KA::DB::Result::Map::Body::Planet')) {
...
sub _view_fleets {
    ...
    if ($target->isa('KA::DB::Result::Map::Planet') and $target->empire_id ~~ \@ally_ids) {
...
sub rename_fleet {
    ...
    KA::Verify->new(content => \$args{name}, throws => [1005, 'Invalid name for a fleet.'])
        ->not_empty
        ->no_profanity
        ->length_lt(31)
        ->only_ascii
