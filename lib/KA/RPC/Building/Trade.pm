...
sub model_class {
    return 'KA::DB::Result::Building::Trade';
}
...
sub accept_from_market {
    ...
    my $offer_fleet = KA->db->resultset('Fleet')->find($trade->fleet_id);
...
sub create_supply_chain {
    ...
    if ($target->empire_id != $empire->id) {
        if ($target->class eq 'KA::DB::Result::Map::Body::Planet::Station') {
...
sub get_trade_fleets {
    ...
    my $target = KA->db->resultset('Map::Body')->find($args->{target_id}) if $args->{target_id};
    my @fleets;
...
sub get_waste_fleets {
    ...
    # get the local star
    my $target      = KA->db->resultset('Map::Star')->find($building->body->star_id);
    my @fleets;
...
sub delete_supply_chain {
    ...
    my $chain = KA->db->resultset('SupplyChain')->find($args->{supply_chain_id});
    if ($chain) {
