package KA::RPC::Building::BlackHoleGenerator;
use Moose;
use utf8;
...
sub model_class {
    return 'KA::DB::Result::Building::Permanent::BlackHoleGenerator';
}
...
#New Method
        ...
        my @bodies = shuffle $db->resultset('Map::Body')->search(
            {
                'me.zone'           => $target_params->{zone},
                'me.empire_id'      => undef,
                'me.class'          => { like => 'KA::DB::Result::Map::Body::Planet::%' },
        ...
        if (@bodies > 0) {
            foreach my $candidate (@bodies) {
                next if ($candidate->get_buildings_of_class('KA::DB::Result::Building::Permanent::Fissure'));
                $target = $candidate;
                last;
            }
        }
...
sub check_member_laws {
    ...
    if (ref $target eq 'HASH') {
        ...
        if ($target->isa('KA::DB::Result::Map::Star')) {
            $btype = 'star';
            $tstar = $target;
        }
        else {
    ...
    if ($task->{name} eq "Jump Zone" or $task->{name} eq "Swap Places") {
        ...
        my $bodies = KA->db->resultset('Map::Body')->search({star_id => [ $body->star_id ]});
        my $throw; my $reason;
        while (my $cbody = $bodies->next) {
            ($throw, $reason) = check_bentry($cbody, $tstar);
            return $throw, $reason if $throw;
        }
        $bodies = KA->db->resultset('Map::Body')->search({star_id => [ $tstar->id ]});
...
sub check_neutral_violation {
    ...
    my $throw; my $reason;
    my $nz_param = KA->config->get('neutral_area');
...
# Swap in happening fully inside or fully outside Neutral Area
    ...
    if ($task->{name} eq "Jump Zone" or $task->{name} eq "Swap Places") {
        ...
        my $bodies = KA->db->resultset('Map::Body')->search({star_id => [ $test_body->star_id ]});
        my $fail = 0;
...
sub generate_singularity {
    ...
    if ( $task->{name} eq "Change Type" && defined ($tempire) ) {
        ...
        if ($btype eq 'asteroid') {
            if ($class eq 'KA::DB::Result::Map::Body::Asteroid::A'.$ntype) {
                confess [1013, "That body is already that type."];
            }
        }
        elsif ($btype eq 'habitable planet') {
            if ($class eq 'KA::DB::Result::Map::Body::Planet::P'.$ntype) {
                confess [1013, "That body is already that type."];
            }
        }
        else {
        ...
        if ($body->get_buildings_of_class('KA::DB::Result::Building::Permanent::Fissure')) {
            $confess = sprintf("%s can not be moved without tearing apart from the fissure on it.", $body->name);
            $allowed = 0;
        }
        elsif ($btype eq 'habitable planet' and $target->get_buildings_of_class('KA::DB::Result::Building::Permanent::Fissure')) {
        ...
        unless ($target->isa('KA::DB::Result::Map::Star')) {
...
#Check for enemy spies.
    ...
    if (defined $lock_down) {
        ...
        my $defense = 0;
        my $hq = $body->get_building_of_class('KA::DB::Result::Building::Security');
        if (defined $hq) {
...
sub qualify_moving_sys {
    ...
    while (my $body = $bodies->next) {
        ...
        if ($body->get_type ne 'asteroid' and $body->get_buildings_of_class('KA::DB::Result::Building::Permanent::Fissure')) {
...
sub make_orbit_array {
    my ($star) = @_;
    my $bodies = KA->db->resultset('Map::Body')->search({star_id => $star->id});
    my @orbits;
    while (my $body = $bodies->next) {
...
sub bhg_swap {
    ...
    unless ($new_data->{type} eq "empty") {
        ...
        if (defined($target->empire)) {
            my $toracle = $body->get_building_of_class('KA::DB::Result::Building::Permanent::OracleOfAnid');
            if ($toracle) {
    ...
    if ($body->get_type ne 'asteroid') {
        ...
        my $boracle = $body->get_building_of_class('KA::DB::Result::Building::Permanent::OracleOfAnid');
        if ($boracle) {
...
sub bhg_make_planet {
    ...
    if ($random < 5) {
        $class = 'KA::DB::Result::Map::Body::Planet::GasGiant::G'.randint(1,KA::DB::Result::Map::Body->gas_giant_types);
        $size  = randint(90, 121);
    }
    else {
...
sub bhg_random_type {
    ...
    if ($throw > 0) {
        ...
        my $params = { newtype => randint(1,KA::DB::Result::Map::Body->planet_types) };
        $body->add_news(50, '%s has gone thru extensive changes.', $target->name);
        $return = bhg_change_type($target, $params);
    }
    elsif ($btype eq 'asteroid') {
        my $params = { newtype => randint(1,KA::DB::Result::Map::Body->asteroid_types) };
        $body->add_news(50, '%s has gone thru extensive changes.', $target->name);
        $return = bhg_change_type($target, $params);
    }
    else {
...
sub bhg_self_destruct {
    ...
    $body->needs_recalc(1);
    $body->update;
    $building->update({class=>'KA::DB::Result::Building::Permanent::Fissure'});
    $body->empire->add_medal('Fissure');
...
sub recalc_miners {
    ...
    for my $body_id (keys %mining_bodies) {
        ...
        my $building = $body->get_building_of_class('KA::DB::Result::Building::Ore::Ministry');
...
sub bhg_change_type {
    ...
    my $btype = $body->get_type;
    if ($btype eq 'asteroid') {
        if ($params->{newtype} >= 1 and $params->{newtype} <= KA::DB::Result::Map::Body->asteroid_types) {
            $class = 'KA::DB::Result::Map::Body::Asteroid::A'.$params->{newtype};
        }
        else {
        ...
        if ($params->{newtype} >= 1 and $params->{newtype} <= KA::DB::Result::Map::Body->planet_types) {
            $class = 'KA::DB::Result::Map::Body::Planet::P'.$params->{newtype};
            $old_class =~ /::(P\d+)/;
            my $old_type = $1;
            $class =~ /::(P\d+)/;
            my $new_type = $1;
