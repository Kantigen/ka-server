package KA::RPC::Building::Parliament;
...
sub model_class {
    return 'KA::DB::Result::Building::Module::Parliament';
}
...
sub max_members {
    ...
    my $embassy = KA->db->resultset('KA::DB::Result::Building')->search(
        { body_id => { in => \@planet_ids }, class => 'KA::DB::Result::Building::Embassy' }, 
        { order_by => { -desc => 'level' } }
    )->first;
...
*view_propositions = \&KA::RPC::Building::Embassy::view_propositions;
...
sub get_mining_platforms_for_asteroid_in_jurisdiction {
    ...
    my $asteroid = KA->db->resultset('KA::DB::Result::Map::Body')->find($asteroid_id);
    ...
    my $platforms = KA->db->resultset('KA::DB::Result::MiningPlatforms')->search({asteroid_id => $asteroid->id});
...
# this is moving to the new location, but keep it available here
# until we're ready to remove it.
*cast_vote = \&KA::RPC::Building::Embassy::cast_vote;
...
sub propose_fire_bfg {
    ...
    KA::Verify->new(content=>\$reason, throws=>[1005,'Reason cannot be empty.',$reason])->not_empty;
    KA::Verify->new(content=>\$reason, throws=>[1005,'Reason cannot contain HTML tags or entities.',$reason])->no_tags;
    KA::Verify->new(content=>\$reason, throws=>[1005,'Reason cannot contain profanity.',$reason])->no_profanity;
    unless ($body_id) {
    ...
    unless ($body->isa('KA::DB::Result::Map::Body::Planet')) {
...
sub propose_writ {
    ...
    KA::Verify->new(content=>\$title, throws=>[1005,'Title cannot be empty.',$title])->not_empty;
    KA::Verify->new(content=>\$title, throws=>[1005,'Title cannot contain any of these characters: {}<>&;@',$title])->no_restricted_chars;
    KA::Verify->new(content=>\$title, throws=>[1005,'Title must be less than 30 characters.',$title])->length_lt(30);
    KA::Verify->new(content=>\$title, throws=>[1005,'Title cannot contain profanity.',$title])->no_profanity;
...
sub propose_transfer_station_ownership {
    ...
    my $to_empire = KA->db->resultset('KA::DB::Result::Empire')->find($to_empire_id);
...
sub propose_rename_star {
    ...
    KA::Verify->new(content=>\$star_name, throws=>[1000,'Name not available.',$star_name])
        ->length_gt(2)
        ->length_lt(31)
        ->no_restricted_chars
        ->no_profanity
        ->no_padding
        ->not_ok(KA->db->resultset('KA::DB::Result::Map::Star')->search({name=>$star_name, 'id'=>{'!='=>$star->id}})->count); # name available
    my $proposition = KA->db->resultset('KA::DB::Result::Propositions')->new({
        type            => 'RenameStar',
...
sub propose_broadcast_on_network19 {
    ...
    KA::Verify->new(content=>\$message, throws=>[1005,'Message cannot be empty.',$message])->not_empty;
...
sub propose_rename_asteroid {
    ...
    KA::Verify->new(content=>\$name, throws=>[1000,'Name not available.',$name])
        ->length_gt(2)
        ->length_lt(31)
        ->no_restricted_chars
        ->no_profanity
        ->no_padding
        ->not_ok(KA->db->resultset('KA::DB::Result::Map::Body')->search({name=>$name, 'id'=>{'!='=>$asteroid->id}})->count); # name available
    my $proposition = KA->db->resultset('KA::DB::Result::Propositions')->new({
        type            => 'RenameAsteroid',
...
sub propose_rename_uninhabited {
    ...
    KA::Verify->new(content=>\$name, throws=>[1000,'Name not available.',$name])
        ...
        ->no_profanity
        ->no_padding
        ->not_ok(KA->db->resultset('KA::DB::Result::Map::Body')->search({name=>$name, 'id'=>{'!='=>$planet->id}})->count); # name available
    my $proposition = KA->db->resultset('KA::DB::Result::Propositions')->new({
...
sub propose_evict_mining_platform {
    ...
    my $platform = KA->db->resultset('KA::DB::Result::MiningPlatforms')->find($platform_id);
...
sub propose_evict_excavator {
    ...
    my $excav = KA->db->resultset('KA::DB::Result::Excavators')->find($excav_id);
...
sub propose_induct_member {
    ...
    my $invite_empire = KA->db->resultset('KA::DB::Result::Empire')->find($empire_id);
...
sub propose_expel_member {
    ...
    my $empire_to_remove = KA->db->resultset('KA::DB::Result::Empire')->find($empire_id);
...
sub view_taxes_collected {
    ...
    my $building = $session->current_building;
    my @out;
    my $taxes = KA->db->resultset('KA::DB::Result::Taxes')->search({station_id => $building->body_id});
