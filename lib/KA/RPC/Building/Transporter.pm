package KA::RPC::Building::Transporter;
...
with 'KA::Role::TraderRpc','KA::Role::Fleet::Trade';
...
sub model_class {
    return 'KA::DB::Result::Building::Transporter';
}
        ...
        search(
               {
                   ...
                   '_buildings.class' => 'KA::DB::Result::Building::Transporter',
                   '_buildings.level'      => { '>' => 0 },
...
sub push_items {
    ...
    unless ($target->empire_id == $empire->id || ($target->class eq 'KA::DB::Result::Map::Body::Planet::Station' && $target->alliance_id == $empire->alliance_id)) {
        confess [1010, 'You cannot push items to a planet that is not your own.'];
    }
    my $transporter = $target->get_building_of_class('KA::DB::Result::Building::Transporter');
