package KA::RPC::Building::TheDillonForge;
...
sub model_class {
    return 'KA::DB::Result::Building::Permanent::TheDillonForge';
}
...
PLAN:
    for my $plan (@{$body->plan_cache}) {
        ...
        my $glyphs = KA::DB::Result::Plan->get_glyph_recipe($plan->class);
        next PLAN if not $glyphs;

        my ($class) = $plan->class =~ m/KA::DB::Result::Building::(.*)$/;
