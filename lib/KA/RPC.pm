...
use KA::Util qw(format_date real_ip_address);
use Log::Any qw($log);
...
sub get_session {
    ...
    if (ref $session ne 'KA::Session') {
        if (ref $session eq 'KA::DB::Result::Empire') {
            ...
            $session = KA::Session->new(id=>$session);
    ...
    if ($opts->{empire_id}) {
        ...
        if ($session->current_body->isa('KA::DB::Result::Map::Body::Planet::Station'))
        {
    ...
    if (defined $empire) {
        if (!$session->rpc_counted) {
            ...
            my $throttle = KA->config->get('rpc_throttle') || 30;
            if (my $delay = KA->cache->get('rpc_block', $opts->{session_id})) {
...
sub get_baby {
    ...
    if (ref $empire_id && $empire_id->isa('KA::DB::Result::Empire')) {
        return $empire_id;
    }
...
sub get_body { # makes for uniform error handling, and prevents staleness
    ...
    if (ref $body_id && $body_id->isa('KA::DB::Result::Map::Body')) {
        return $body_id;
    }
...
sub get_building { # makes for uniform error handling, and prevents staleness
    ...
    if (ref $building_id) {
        if (blessed($building_id) && $building_id->isa('KA::DB::Result::Building')) {
            return $building_id;
        }
        # how do we get here?
        ...
        search(
               {
                   ...
                   -or => [
                           ...
                           $session->empire->alliance_id ? {
                               ...
                               'body.class'            => 'KA::DB::Result::Map::Body::Planet::Station'
                           } : (),
                           $session->is_sitter ? () : {
                               'sitterauths.sitter_id' => $session->empire->id,
                               'body.class' => { '!=' => 'KA::DB::Result::Map::Body::Planet::Station' },
...
sub format_status {
    ...
    my %out = (
        server  => {
            ...
            version         => KA->version,
            star_map_size   => KA->config->get('map_size'),
            rpc_limit       => KA->config->get('rpc_limit') || 2500,
        },
    );

    # can just pass in the session, we'll extract the body and empire from it
    my $real_empire;
    if ($empire->isa('KA::Session')) {
    ...
    if (defined $real_empire) {
        ...
        if (my @promos = KA->db->resultset('Promotion')->current_promotions) {
