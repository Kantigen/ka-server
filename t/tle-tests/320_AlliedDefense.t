...
my @asteroids = KA->db->resultset('KA::DB::Result::Map::Body')->search(
	{ zone => $testers[0]->empire->home_planet->zone, size => 10, class => { like => 'KA::DB::Result::Map::Body::Asteroid%' } },
)->all();

my @planets = KA->db->resultset('KA::DB::Result::Map::Body')->search(
	{ zone => $testers[0]->empire->home_planet->zone, orbit => 3, class => { like => 'KA::DB::Result::Map::Body::Planet::P%'} },
)->all();
...
for my $tester ( @testers ) {
	...
	my $miningmin = KA::db->resultset('KA::DB::Result::Building')->new({
			x       => -2,
			y       => 5,
			class   => 'KA::DB::Result::Building::Ore::Ministry',
			level   => 10,
			});
	$home->build_building($miningmin);
	$miningmin->finish_upgrade;

	my $munitions = KA::db->resultset('KA::DB::Result::Building')->new({
			x       => -1,
			y       => 5,
			class   => 'KA::DB::Result::Building::MunitionsLab',
			level   => 10,
			});
	$home->build_building($munitions);
	$munitions->finish_upgrade;

	my $observatory = KA::db->resultset('KA::DB::Result::Building')->new({
			x       => 0,
			y       => 5,
			class   => 'KA::DB::Result::Building::Observatory',
			level   => 10,
			});
	$home->build_building($observatory);
	$observatory->finish_upgrade;

	my $saw = KA::db->resultset('KA::DB::Result::Building')->new({
			x       => 1,
			y       => 5,
			class   => 'KA::DB::Result::Building::SAW',
			level   => 10,
			});
	...
	my $fighter = KA->db->resultset('KA::DB::Result::Ships')->new({type=>'fighter'});
	$shipyard->build_ship($fighter);

	my $fighter2 = KA->db->resultset('KA::DB::Result::Ships')->new({type=>'fighter'});
	$shipyard->build_ship($fighter2);

	my $fighter3 = KA->db->resultset('KA::DB::Result::Ships')->new({type=>'fighter'});
	$shipyard->build_ship($fighter3);

	my $sweeper = KA->db->resultset('KA::DB::Result::Ships')->new({type=>'sweeper'});
	$shipyard->build_ship($sweeper);

	my $sweeper2 = KA->db->resultset('KA::DB::Result::Ships')->new({type=>'sweeper'});
	$shipyard->build_ship($sweeper2);

	my $detonator = KA->db->resultset('KA::DB::Result::Ships')->new({type=>'detonator'});
	$shipyard->build_ship($detonator);

	my $detonator2 = KA->db->resultset('KA::DB::Result::Ships')->new({type=>'detonator'});
	$shipyard->build_ship($detonator2);

	my $miningship = KA->db->resultset('KA::DB::Result::Ships')->new({type=>'mining_platform_ship'});
	$shipyard->build_ship($miningship);

	my $miningship2 = KA->db->resultset('KA::DB::Result::Ships')->new({type=>'mining_platform_ship'});
	$shipyard->build_ship($miningship2);

	my $stake = KA->db->resultset('KA::DB::Result::Ships')->new({type=>'stake'});
	$shipyard->build_ship($stake);

	my $colonyship = KA->db->resultset('KA::DB::Result::Ships')->new({type=>'colony_ship'});
...
for my $i ( 0 .. 1 ) {
	...
	my $probe = KA->db->resultset('KA::DB::Result::Ships')->search({body_id => $tester{home}->id, type=>'probe'})->first;
	...
	$probe = KA->db->resultset('KA::DB::Result::Ships')->search({id=>$probe->id})->first; # pull the latest data on this ship
	$probe->arrive;

	# Send a mining platform ship to the closest asteroid
	my $miningship = KA->db->resultset('KA::DB::Result::Ships')->search({body_id => $tester{home}->id, type=>'mining_platform_ship'})->first;
	...
	$miningship = KA->db->resultset('KA::DB::Result::Ships')->search({id=>$miningship->id})->first; # pull the latest data on this ship
	$miningship->arrive;

	# Send a fighter to defend the closest asteroid
	my $fighter = KA->db->resultset('KA::DB::Result::Ships')->search({body_id => $tester{home}->id, type=>'fighter', task=>'Docked'})->first;
    ...
    $fighter = KA->db->resultset('KA::DB::Result::Ships')->search({id=>$fighter->id})->first; # pull the latest data on this ship
    $fighter->arrive;
	...
	my $fighter2 = KA->db->resultset('KA::DB::Result::Ships')->search({body_id => $tester{home}->id, type=>'fighter', task=>'Docked'})->first;
    ...
    $fighter2 = KA->db->resultset('KA::DB::Result::Ships')->search({id=>$fighter2->id})->first; # pull the latest data on this ship
    $fighter2->arrive;
...
for my $i ( 0 .. 1 ) {
	...
	my $sweeper = KA->db->resultset('KA::DB::Result::Ships')->search({body_id => $tester{home}->id, type=>'sweeper'})->first;
    ...
    $sweeper = KA->db->resultset('KA::DB::Result::Ships')->search({id=>$sweeper->id})->first; # pull the latest data on this ship
    $sweeper->arrive;
	...
	my $fighter3 = KA->db->resultset('KA::DB::Result::Ships')->search({body_id => $tester{home}->id, type=>'fighter', task=>'Docked'})->first;
    ...
    $fighter3 = KA->db->resultset('KA::DB::Result::Ships')->search({id=>$fighter3->id})->first; # pull the latest data on this ship
    $fighter3->arrive;
	...
	my $fighter2 = KA->db->resultset('KA::DB::Result::Ships')->search({id=>$enemy->{fighter2}})->first;
	...
	my $stake = KA->db->resultset('KA::DB::Result::Ships')->search({body_id => $tester{home}->id, type=>'stake'})->first;
	...
	$fighter2 = KA->db->resultset('KA::DB::Result::Ships')->search({id=>$enemy->{fighter2}})->first;
	diag "Enemy fighter 2 is at ", $fighter2->foreign_body_id, " task ", $fighter2->task, " combat ", $fighter2->combat;
    $fighter2->arrive;
	$fighter2 = KA->db->resultset('KA::DB::Result::Ships')->search({id=>$enemy->{fighter2}})->first;
    $fighter2->arrive;
	...
	my $colony_ship = KA->db->resultset('KA::DB::Result::Ships')->search({body_id => $tester{home}->id, type=>'colony_ship'})->first;
	...
	$colony_ship = KA->db->resultset('KA::DB::Result::Ships')->search({id=>$colony_ship->id})->first; # pull the latest data on this ship
	$colony_ship->arrive;
	...
	my $detonator = KA->db->resultset('KA::DB::Result::Ships')->search({body_id => $tester{home}->id, type=>'detonator'})->first;
	...
	$detonator = KA->db->resultset('KA::DB::Result::Ships')->search({id=>$detonator->id})->first; # pull the latest data on this ship
	$detonator->arrive;

	$result = $tester->post('spaceport', 'view', [$tester{session_id}, $tester->{spaceport_id}]);
	is( $result->{result}{status}{empire}{most_recent_message}{subject}, "Detonator Report", 'Detonator took out probes' );

	my $detonator2 = KA->db->resultset('KA::DB::Result::Ships')->search({body_id => $tester{home}->id, type=>'detonator'})->first;
	...
	diag "Detonator arrives ", $result->{result}{ship}{date_arrives};
	ok($result->{result}{ship}{date_arrives}, "Detonator sent");
	$detonator2 = KA->db->resultset('KA::DB::Result::Ships')->search({id=>$detonator2->id})->first; # pull the latest data on this ship
	$detonator2->arrive;
