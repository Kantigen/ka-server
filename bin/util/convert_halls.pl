...
while (my $body = $body_rs->next) {
    ...
    my @halls = $body->get_buildings_of_class('KA::DB::Result::Building::Permanent::HallsOfVrbansk');
    my $stats_hall_buildings = scalar(@halls);
    my $hall_plans = $body->get_plan('KA::DB::Result::Building::Permanent::HallsOfVrbansk', 1);
    ...
    if ($stats_hall_buildings) {
        ...
        $body->add_plan('KA::DB::Result::Building::Permanent::HallsOfVrbansk',1,0,$stats_hall_buildings);
    }
}
