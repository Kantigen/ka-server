...
my $abandons_tally;
my $inactivity_time_out = KA->config->get('self_destruct_after_inactive_days') || 20;
...
while (my $empire = $inactives->next) {
    ...
    if ($empire->essentia >= 1) {
      unless (KA->cache->get('empire_inactive',$empire->id)) {
        ...
        $empire->update;
        KA->cache->set('empire_inactive',$empire->id,1,60*60*24);
      }
    }
    else {
